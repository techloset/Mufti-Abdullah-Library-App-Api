...
const MonthlyCard: React.FC<Book> =({thumbnail,title,amount,id,details,author})=>{
    ...
    return (
        <div className="relative  w-[270px] h-[336px] sm:w-[358px] sm:h-[260px] rounded-md md:flex md:flex-row border-2 p-2"  onClick={handleClick}>
            ...
            <div className="mt-[132px] md:mt-0 md:flex-1 p-4 flex flex-col justify-around text-center md:text-left">
                <div>
                    <p className="font-semibold italic text-[22px] text-wrap text-[#183B56] mb-2"style={{fontFamily:"Hanken Grotesk" }}>{title}</p>
                    <p className="font-semibold  text-[16px] text-wrap text-[#183B56]"style={{fontFamily:"Hanken Grotesk" }}>{author}</p>
                    <p className="font-bold  text-[22px] text-wrap text-[#1565D8] mt-2"style={{fontFamily:"Hanken Grotesk" }}>{amount}</p>
                </div>
                <div className="flex justify-between items-center mt-2 md:mt-3">
                    <button className="rounded-full bg-[#183B56] font-semibold sm:w-[101px] md:w-[70px] lg:w-[101px] text-center p-2 text-[white] sm:text-[14px] md:text-[10px] lg:text-[14px]">Buy Now</button>
